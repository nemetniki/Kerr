...
parser.add_argument("chipp",type=float,help='chipp: Kerr nonlinearity coefficient')
...
################################
initKerr     = np.zeros(N_env+1,complex)
if args.cohC>0.:
	initKerr = coherentS(np.sqrt(args.cohC),initKerr)
else:
	initKerr[args.init_ind]  = 1. #starting at |e>
...
ind_sys       = L
states[ind_sys] = initKerr/np.sqrt(np.sum(initKerr**2))

NB_outa = 0.
normL = 1.

ndiag = np.linspace(0,N_env,N_env+1).astype(np.int64)
n = np.diag(ndiag)
n2 = np.diag(ndiag-1)
g2op = n*n2
...
NB    = np.einsum("ij,ij",Bdens,n)
g2_t = np.einsum("ij,ij",Bdens,g2op)/NB**2
...
if args.cohC>0:
	filename = "./Data/Kerr/evol%04d_coherent_cavity.txt" % (args.ID)
	outname = "./Data/Kerr/OUT%04d_coherent_cavity.txt" % (args.ID)
	specname = "./Data/Kerr/spec%04d_coherent_cavity.txt" % (args.ID)
	g2tau = "./Data/Kerr/g2tau%04d_coherent_cavity.txt" % (args.ID)
	corrf = "./Data/Kerr/corr%04d_coherent_cavity.txt" % (args.ID)
	Bdenspath = "./Data/Kerr/Bdens%04d_coherent_cavity" % (args.ID)
	Cdenspath = "./Data/Kerr/Cdens%04d_coherent_cavity" % (args.ID)
elif args.cohE>0:
	filename = "./Data/Kerr/evol%04d_coherent_environment.txt" % (args.ID)
	outname = "./Data/Kerr/OUT%04d_coherent_environment.txt" % (args.ID)
	specname = "./Data/Kerr/spec%04d_coherent_environment.txt" % (args.ID)
	g2tau = "./Data/Kerr/g2tau%04d_coherent_environment.txt" % (args.ID)
	corrf = "./Data/Kerr/corr%04d_coherent_environment.txt" % (args.ID)
	Bdenspath = "./Data/Kerr/Bdens%04d_coherent_environment" % (args.ID)
	Cdenspath = "./Data/Kerr/Cdens%04d_coherent_environment" % (args.ID)
elif Omc>0:
	filename = "./Data/Kerr/evol%04d_cavity_drive.txt" % (args.ID)
	outname = "./Data/Kerr/OUT%04d_cavity_drive.txt" % (args.ID)
	specname = "./Data/Kerr/spec%04d_cavity_drive.txt" % (args.ID)
	g2tau = "./Data/Kerr/g2tau%04d_cavity_drive.txt" % (args.ID)
	corrf = "./Data/Kerr/corr%04d_cavity_drive.txt" % (args.ID)
	Bdenspath = "./Data/Kerr/Bdens%04d_cavity_drive" % (args.ID)
	Cdenspath = "./Data/Kerr/Cdens%04d_cavity_drive" % (args.ID)
else:
	filename = "./Data/Kerr/evol%04d.txt" % (args.ID)
	outname = "./Data/Kerr/OUT%04d.txt" % (args.ID)
	specname = "./Data/Kerr/spec%04d.txt" % (args.ID)
	g2tau = "./Data/Kerr/g2tau%04d.txt" % (args.ID)
	corrf = "./Data/Kerr/corr%04d.txt" % (args.ID)
	Bdenspath = "./Data/Kerr/Bdens%04d" % (args.ID)
	Cdenspath = "./Data/Kerr/Cdens%04d" % (args.ID)
...
file_out.write("""ID = %04d\ngamma_L = %f, gamma_R = %f, Om_c = %f, phi = %fpi,
Delta_c = %f, g = %f, Nphot_max = %d, initind = %d,
tolerance = %.0E, delay_L = %d, endt = %.0f, dt = %f\n
coherent initial state amplitude for cavity: %f and the environment %f
Data file: M*dt,norm,nc_exp,g2_ta,g2_t,NB\n""" % (args.ID,gamma_L, gamma_R, Omc, args.phi, Delc, chipp, N_env, args.init_ind, Decimal(tol), L, endt, dt,args.cohC,args.cohE))
file_out.close()
...
#		print("ind_sys-1",states[ind_sys-1].shape)
	...
	nc_exp = exp_sys(n,states[ind_sys])
	g2_ta =  exp_sys(g2op,states[ind_sys])/nc_exp**2
	file_evol.write("%.20f\t%.20f\t%.20f\t%.20f\t%.20f\t%.20f\n" %(M*dt,norm,nc_exp,g2_ta,g2_t,NB))
	file_evol.flush()
	file_out.close()
	...
	# Unitary step #
	#%%%%%%%%%%%%%%#
	U_block = U(initenv,states[ind_sys],states[ind_sys-1],M,gamma_L,gamma_R,dt,phi,Omc,Delc,chipp)
...
nc_exp = exp_sys(n,states[N-1])
g2_ta  = exp_sys(g2op,states[N-1])
