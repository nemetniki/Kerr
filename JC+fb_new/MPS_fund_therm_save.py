...
#################################################
#@profile
def SVD_sig(block,cutoff,M=0,N=0,L=50):
...
#	svd_init  = None
	...
	if M == int((N-L)/2):
		print(" N = ",N," L = ",L," M = ",M," int((N-L)/2) = ",int((N-L)/2))
		matname = "./mat.txt"
		mheader = "shape of array: " + np.array2string(np.array(block.shape)) + "\n Number of non-zeros singular values: " + str(d)
		np.savetxt(matname, block,header=mheader)
...
####################################
#@profile
def cut(M,N,L,block,tolerance,how,OC=None):
	...
	# separate the block
	block_merged_svd,link_dim = SVD_sig(block_merged,tolerance,M,N,L)
...
#	print("L = ", L )
	...
	for s in range(0,L-1):
		...
		if direction=="future":
			states[base_ind+1+c*s],states[base_ind+c*s] = cut(M,N,L,swap_block,tol,"both","left")     
		elif direction=="past":
			states[base_ind+1+c*s],states[base_ind+c*s] = cut(M,N,L,swap_block,tol,"both","right")     
