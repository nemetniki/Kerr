...
#    print(M*dt)
	...
	if M>0:
		...
		states[M],states[M-1] = OC_reloc(file_k,states[M],states[M-1],"left",tol)
		states[M:M+L] = SWAP(file_k,states,M,"future",L,tol)
    ...
    # operator's action if applicable
		states[ind_sys],states[ind_sys-1] = OC_reloc(file_k,states[ind_sys],states[ind_sys-1],"left",tol)
...
#    print("U block merge right",U_block.shape)
	...
	#print("main M = ",M)
	states[ind_sys+1],U_small_block = cut(file_k,U_block,tol,"right")
#    print("tS and rest",states[ind_sys+1].shape,U_small_block.shape)
	...
	states[ind_sys],states[ind_sys-1] = cut(file_k,U_small_block,tol,"left")
...
#    print("U done, states done", states[ind_sys+1].shape, states[ind_sys].shape, states[ind_sys-1].shape)
	...
	states[(ind_sys-L):(ind_sys)] = SWAP(file_k,states,(ind_sys-2),"past",L,tol)
...
states[M+1],states[M] = OC_reloc(file_k,states[M+1],states[M],"left",tol)
...
for j in range(L):
	states[N-L+j],states[N-L-1+j] = OC_reloc(file_k,states[N-L+j],states[N-L+j-1],"left",tol)
